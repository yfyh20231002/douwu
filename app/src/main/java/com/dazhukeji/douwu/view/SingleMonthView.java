...
public class SingleMonthView extends MonthView {
    ...
    protected boolean onDrawSelected(Canvas canvas, Calendar calendar, int x, int y, boolean hasScheme) {
        int cx = x + mItemWidth / 6;
        int cy = y + mItemHeight / 6;
        if (calendar.isCurrentDay()){
            Bitmap bitmap= BitmapFactory.decodeResource(getResources(), R.drawable.icon_calendar_current);
            canvas.drawBitmap(bitmap,cx,cy,null);
        }else {
            Bitmap bitmap= BitmapFactory.decodeResource(getResources(), R.drawable.icon_calendar_select);
            canvas.drawBitmap(bitmap,cx,cy,null);
        }
